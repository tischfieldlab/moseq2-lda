...
def plot_lda_projection(lda_transformed, group_vals, groups, palette, markers, title="LDA", ax=None, style=None):
    ...
    if style == 'kde':
        sns.kdeplot(x=lda_transformed[:, 0], y=lda_transformed[:, 1], hue=group_vals, hue_order=groups, palette=palette, fill=True, alpha=0.2, ax=ax, legend=False)
        sns.scatterplot(x=lda_transformed[:, 0], y=lda_transformed[:, 1], hue=group_vals, hue_order=groups, style=group_vals, style_order=groups, markers=markers, palette=palette, legend=False, ax=ax)

    elif style == 'centroid':
        desat_palette = sns.color_palette(palette, desat=0.3)
        sns.scatterplot(x=lda_transformed[:, 0], y=lda_transformed[:, 1], hue=group_vals, hue_order=groups, style=group_vals, style_order=groups, markers=markers, palette=desat_palette, legend=False, alpha=0.5, ax=ax)
        gmeans = []
        for g in groups:
            gmeans.append(
                lda_transformed[group_vals == g].mean(axis=0)
            )
        gmeans = np.array(gmeans)
        # print(lda_transformed, group_vals, groups, gmeans, gmeans.shape, group_vals == g)
        ax = sns.scatterplot(x=gmeans[:, 0], y=gmeans[:, 1], hue=groups, hue_order=groups, style=groups, style_order=groups, markers=markers, palette=palette, legend='full', alpha=1.0, s=420, ax=ax)
...
def plot_lda_confusion_matrix(lda_predictions, group_vals, groups, ax=None):
    ...
    sns.heatmap(confusion, ax=ax, xticklabels=groups, yticklabels=groups, cbar_kws={'label':'Row Normalized Confusion'}, vmin=0, vmax=1, annot=True)
...
def plot_lda_weights(lda_coeff, groups, relative_weights=None, ax=None):
    ...
    coef_heat_kwargs = {
        'ax': ax, 'xticklabels': groups, 'cbar_kws': {'label':'Feature Weights'}
    }
...
def plot_lda_scalings(lda, ax=None):
    ...
    sm.set_array([])
    cax = inset_axes(ax, width='3%', height='100%', loc='lower left', bbox_to_anchor=(1.05, 0, 1, 1), bbox_transform=ax.transAxes, borderpad=0,)
...
def plot_lda_results_2D(lda, lda_transformed, lda_predictions, group_vals, groups, palette, markers, title="LDA", figsize=(25, 25), relative_weights=None):

    fig, axs = plt.subplots(2, 2, figsize=figsize)

    plot_lda_projection(lda_transformed, group_vals, groups, palette=palette, markers=markers, title=title, ax=axs[0, 0], style='kde')

    plot_lda_projection(lda_transformed, group_vals, groups, palette=palette, markers=markers, title=title, ax=axs[0, 1], style='centroid')

    fig.subplots_adjust(right=0.75)
    sns.move_legend(axs[0, 1], "upper left", bbox_to_anchor=(1.05, 0.5))
    sns.despine(fig=fig)

    plot_lda_confusion_matrix(lda_predictions, group_vals, groups, ax=axs[1,0])
