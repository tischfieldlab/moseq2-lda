...
def plot_lda_projection(lda_transformed, group_vals, groups, palette, markers, title="LDA", ax=None, style=None):
    ...
    if style == 'kde':
        try:
            sns.kdeplot(x=lda_transformed[:, 0],
                        y=lda_transformed[:, 1],
                        hue=group_vals,
                        hue_order=groups,
                        palette=palette,
            ...
            print(e)
            pass
        sns.scatterplot(x=lda_transformed[:, 0],
                        y=lda_transformed[:, 1],
                        hue=group_vals,
                        hue_order=groups,
                        style=group_vals,
...
def plot_lda_confusion_matrix(lda_predictions, group_vals, groups, ax=None):
    ...
    sns.heatmap(confusion,
                ...
                xticklabels=groups,
                yticklabels=groups,
                cbar_kws={'label': 'Row Normalized Confusion'},
                vmin=0,
                vmax=1,
                annot=True)
...
def plot_lda_results_2D(lda, lda_transformed, lda_predictions, group_vals, aes: Aesthetics = None, title="LDA",
                        figsize=(25, 25), relative_weights=None):
    ...
    plot_lda_projection(lda_transformed,
                        group_vals,
                        aes.groups,
                        palette=aes.palette,
                        markers=aes.markers,
