...
def plot_lda_results(
    ...
    for i, m in enumerate(data.meta):
        out_data.append(
            {**asdict(m), "predicted_group": lda_predictions[i], **{f"LDA_{d+1}": lda_result[i, d] for d in range(lda.lda.n_components)}}
        )
...
def plot_permutation_score(lda, X, y, cv=None, n_permutations=1000, ax=None):
    ...
    score, perm_scores, pvalue = permutation_test_score(lda, X, y, scoring="accuracy", cv=cv, n_permutations=n_permutations)
