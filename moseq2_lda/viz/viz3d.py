...
def plot_lda_kde_projections_3D(ax, lda_transformed, groups, group_vals, palette, levels=5, alpha=0.5):
    ...
    for g in groups:
        ...
        colors = sns.light_palette(palette[groups.index(g)], levels+1)
...
def plot_lda_kde_projections_3D_iso(ax, lda_result, groups, group_vals, palette, levels=5, alpha=0.5):
    minx, maxx = ax.get_xlim()
    miny, maxy = ax.get_ylim()
    minz, maxz = ax.get_zlim()
...
def plot_lda_results_3D(coeff, lda_result, lda_predictions, group_vals, groups, palette, markers, title, figsize, relative_weights):
